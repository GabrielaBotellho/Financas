...
/**
 * Busca o item (a conexão bancária em si) — pra saber quando foi a última
 * sincronização bem-sucedida (lastUpdatedAt) e se está tudo certo ou com
 * erro (status: UPDATED, LOGIN_ERROR, OUTDATED, etc). Itens do Meu Pluggy
 * sincronizam sozinhos 1x/dia; isso serve pra dar visibilidade de quando
 * foi a última vez de verdade, em vez de assumir que os dados são atuais.
 */
async function fetchItem(itemId) {
  ...
  if (!res.ok) {
    const text = await res.text();
    throw new Error(`Falha ao buscar status da conexão (${res.status}): ${text}`);
  }
  return res.json();
}
